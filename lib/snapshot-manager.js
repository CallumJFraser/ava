...
const concordanceOptions = require('./concordance-options').plain;
...
function decodeSnapshots(buffer) {
...
class Manager {
	...
	compare(options) {
		...
		const actual = concordance.deserialize(snapshotBuffer);

		const expected = concordance.describe(options.expected);
	...
	record(hash, options) {
		const descriptor = concordance.describe(options.expected);
...
function load(dir, name, relFile, updating) {
	...
	if (!updating) {
		...
		if (buffer) {
			snapshotsByHash = decodeSnapshots(buffer);
		} else {
			appendOnly = false;
		}
	}

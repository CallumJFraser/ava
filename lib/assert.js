...
const concordanceOptions = require('./concordance-options').recommended;
const enhanceAssert = require('./enhance-assert');

function formatDescriptorDiff(actualDescriptor, expectedDescriptor) {
	return {
		label: 'Difference:',
		formatted: concordance.diffDescriptors(actualDescriptor, expectedDescriptor, concordanceOptions)
	};
}
...
function formatWithLabel(label, value) {
	return formatDescriptorWithLabel(label, concordance.describe(value));
}
	...
	const assertions = {
		...
		deepEqual(actual, expected, message) {
			...
			if (result.pass) {
				...
				const actualDescriptor = result.actual || concordance.describe(actual);
				const expectedDescriptor = result.expected || concordance.describe(expected);
				fail(this, new AssertionError({
					assertion: 'deepEqual',
					message,
					values: [formatDescriptorDiff(actualDescriptor, expectedDescriptor)]
